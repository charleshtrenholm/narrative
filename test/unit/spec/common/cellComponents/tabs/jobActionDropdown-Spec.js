...
], (JobActionDropdown, JobMessages, Props, TestAppObj) => {
    'use strict';
        ...
        describe('buttons', () => {
            ...
            it('should perform dropdown options when clicked', function () {
                const validStatuses = {
                    ...
                    retry: ['error', 'terminated'],
                };
                const orderedButtons = [
                    {
                        statusList: ['created', 'estimating', 'queued'],
                        validStatuses: validStatuses['cancel'],
                    },
                    {
                        statusList: ['running'],
                        validStatuses: validStatuses['cancel'],
                    },
                    {
                        ...
                        validStatuses: validStatuses['retry'],
                    },
                    {
                        statusList: ['error'],
                        validStatuses: validStatuses['retry'],
                    ...
                    await jobActionDropdownInstance
                        ...
                        .then((outcome) => {
                            ...
                            expect(this.jobManager.getJobIDsByStatus.calls.allArgs()).toEqual([
                                [['running'], ['created', 'estimating', 'queued', 'running']],
                            ]);
                        });
                });
            });
            ...
            it('resolves to false if the user dismisses the dialog', async function () {
                ...
                await jobActionDropdownInstance
                    ...
                    .then((outcome) => {
                        ...
                        expect(this.jobManager.getJobIDsByStatus).toHaveBeenCalled();
                        expect(this.jobManager.getJobIDsByStatus.calls.allArgs()).toEqual([
                            [[target], ['created', 'estimating', 'queued', 'running']],
                        ]);
